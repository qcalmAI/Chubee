...
>
> Last verified: 2026-06-15 by Chubee (spring cleaning). vLLM build 0.22.2.dev0+g0decac0d9.d20260606.
> gpu-memory-utilization: **0.60** (via VLLM_GPU_MEM_UTIL in .env).
> Only hermes-dashboard container is live -- hermes stopped (dual-container risk).
> gpu-memory-utilization: **0.60** (via `VLLM_GPU_MEM_UTIL` in `.env`).
...
**The default model IS nemotron-nano-30b** (Hermes model.default, provider
local -> http://172.17.0.1:8000/v1). Restored 2026-06-15 during spring cleaning.
The sovereign, free, local default is now live.
...
`hermes-dashboard` (only), `qdrant`, `searxng`, `crawl4ai`, `kiwix`, `kiwix-nginx`,
`filebrowser`, `wyoming-whisper/-openwakeword/-piper`, `portainer`,
`stirling-pdf`. Ollama runs as a **host systemd service** (not Compose),
serving small models only: `bge-m3` (embeddings) + `haervwe/GLM-4.6V-Flash-9B` (vision).

**Samba file share** (`smbd`) on the host at `\\192.168.1.156\PastedImages` — guest
writable, no auth. Bridges the Windows desktop's local clipboard to the GB10 for
pasted images. The Hermes desktop app saves paste images here instead of the local
`C:\Users\...\composer-images\` folder, so the agent can access them from the
remote server. 100% local — uses the direct LAN link (192.168.1.x), not Tailscale.

**Local vision pipeline** — the auxiliary vision model in `~/.hermes/config.yaml`
points explicitly at the local Ollama instance (`haervwe/GLM-4.6V-Flash-9B:latest`
via `http://127.0.0.1:11434/v1`). When a pasted image lands on the share, the
agent calls `vision_analyze` → Ollama GLM-4.6V (8.8 GB GPU memory) → text
description → Nemotron/DeepSeek context. No cloud dependency for image analysis.
...
### 2026-06-15
- **Spring cleaning.** Stopped hermes container (dual-container risk). Restored
  model.default to nemotron-nano-30b/local. Removed stale aliases
  (nemotron-3-super, qwen2.5-32b). Deleted 12 stale Docker images (~63GB).
  Cleaned relic dirs, 19 backup files. Moved youtube-textbook scripts into
  skill. Fast-forwarded chubee-custom through 734 upstream commits.
  Updated this doc: model.default, containers, aliases, verified date.

### 2026-06-14
- **Samba file share set up** at `\\192.168.1.156\PastedImages` — guest-writable SMB
  share bridging the Windows desktop ↔ GB10 gap for pasted images. Allows the agent
  to access images the user pastes into the Hermes desktop app. 100% local (LAN, no
  internet).
- **Local vision pipeline configured.** Auxiliary vision provider changed from
  `auto` (routed to cloud via OpenRouter/Nous) to explicit `custom` → local
  Ollama `haervwe/GLM-4.6V-Flash-9B:latest` at `http://127.0.0.1:11434/v1`.
  Image analysis now runs entirely on the GB10 GPU — zero cloud cost.
- **Ollama upgraded** from v0.24.0 → v0.30.8 to fix GLM-4.6V vision encoder
  crash (SIGSEGV). The old version's llama.cpp didn't support the model's CLIP
  vision encoder.
