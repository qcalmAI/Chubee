...
/personality [name]  Set personality
                     (Detailed identity customization — SOUL.md, personality presets,
                     CLI aliases, limits — lives in references/hermes-identity.md
                     and references/hermes-identity-limits.md.)
...
### Model Routing: Alias Key = Provider's Served Model ID

**Hermes sends the alias KEY name as the model ID in the API request to the provider. The alias's inner `model:` field is IRRELEVANT for routing.** Whatever you name the `model_aliases` key is exactly the string the provider (vLLM, OpenAI-compatible endpoint, etc.) must serve.

```yaml
model_aliases:
  qwen2.5-32b:          # ← THIS is sent as the model ID to the provider
    model: qwen2.5-32b  # ← IGNORED for routing; cosmetic only
    provider: custom:local
```

Consequence: the provider must serve a model whose ID exactly equals the alias key. For a local vLLM server, the alias key must match a value in `--served-model-name`. A mismatch (extra `local/` prefix, typo, stale name) makes routing fail silently and fall through to the default provider — the user sees a 404. Before editing any alias, verify the provider already serves that exact name. See the `vllm-serving` skill's "Local Model Install Checklist" for the full ordered procedure.
...
## Long-Running Jobs: Detach to Survive the Watcher

Hermes' background-process watcher SIGTERMs (-15) tracked `terminal(background=true)`
jobs after `TERMINAL_LIFETIME_SECONDS` (default **300s**; configurable in the agent
`.env`). Long downloads, source builds, and big batch loops get killed mid-run.

**Symptom:** a background job's completion notification reports `exit -15` even though
the real work finished or is still going. (If the job was `nohup`-wrapped — as Hermes'
own bg wrapper does — the child often survives the SIGTERM and keeps running, so the
notification lies. Don't trust `-15`; check the logfile / `ls -lh` the output.)

**Fix — launch fully detached so the watcher has no child to kill:**

```bash
# tmux (most reliable, fully detached, survives everything):
tmux new-session -d -s job1 'wget -c URL -O /path/file 2>&1 | tee /tmp/job1.log'
# poll progress (max 3-min check intervals per user pref):
tmux capture-pane -t job1 -p | tail -5      # or: tail -5 /tmp/job1.log

# setsid alternative (no tmux dependency):
setsid bash -c 'long_command > /tmp/job1.log 2>&1' &
```

**Rules of thumb:**
- Any job expected to exceed ~3 min → detach via `tmux new-session -d` or `setsid`,
  write stdout to a logfile, poll the log. Do NOT rely on plain `background=true` for
  truly long work.
- Raising `TERMINAL_LIFETIME_SECONDS` (e.g. to 7200 = 2h) widens the grace window but
  a genuinely stuck process also lingers longer. Belt-and-suspenders: raise it AND
  detach the multi-hour jobs (model downloads, vLLM source builds).
- Sequential multi-item loops (download A, then B, then C) are especially exposed —
  the whole loop is one tracked job. Detach the loop, or run items as separate
...
(Full skill authoring guide — frontmatter validation, in-repo placement,
Dockerized-Hermes permission workarounds, peer-matched structure —
lives in references/skill-authoring.md.)
```python
import json, os
